...
void saveGaussianPly(const std::string &path, GaussianParams &p, int step) {
    ...
    // Must match the resume parser in loadGaussianPly — a writer/reader mismatch
    // here silently restarts every resumed run's schedules at step 1.
...
LoadedGaussians loadGaussianPly(const std::string &path, float scale, const float translation[3], bool keepCrs) {
    ...
    while (std::getline(f, line)) {
        ...
        // Legacy header written between Jun 24 and Jul 4 2026 ("comment msplat v<step>")
        const std::string legacyIterPrefix = "comment msplat v";
        if (line.rfind(legacyIterPrefix, 0) == 0)
            step = std::stoi(line.substr(legacyIterPrefix.length()));
